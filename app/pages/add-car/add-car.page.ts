...
export class AddCarPage implements OnInit {
  ...
  rate_car(): Promise<number> {
    console.log('random');
    const min = 8;
    const max = 9.3;
    const randomNumber = Math.random() * (max - min) + min;
    this.AI_rate2 = Number(randomNumber.toFixed(2));

    return new Promise<number>((resolve, reject) => {
      let avgKm: any = 0;
      let avgPrice: any = 0;
      this.CarSrv.get_by_model(this.brand[this.AddCarForm.get('Brand')?.value].Models[this.AddCarForm.get('Model')?.value])
        .then((res) => {
          if (res) {
            this.cars = res;
            ...
            resolve(this.AI_rate);
          } else {
            reject(new Error('Failed to get cars'));
          }
        });
    });
  }
